...
brew install anomalyco/tap/eternity-code # macOS 和 Linux（推荐，始终保持最新）
brew install eternity-code              # macOS 和 Linux（官方 brew formula，更新频率较低）
sudo pacman -S eternity-code            # Arch Linux (Stable)
paru -S eternity-code-bin               # Arch Linux (Latest from AUR)
mise use -g eternity-code               # 任意系统
nix run nixpkgs#eternity-code           # 或用 github:anomalyco/eternity-code 获取最新 dev 分支
```
...
OpenCode 也提供桌面版应用。可直接从 [发布页 (releases page)](https://github.com/anomalyco/eternity-code/releases) 或 [eternity-code.ai/download](https://eternity-code.ai/download) 下载。
...
4. `$HOME/.eternity-code/bin` - 默认备用路径
...
了解更多 [Agents](https://eternity-code.ai/docs/agents) 相关信息。

### 文档

更多配置说明请查看我们的 [**官方文档**](https://eternity-code.ai/docs)。
...
如果你在项目名中使用了 “eternity-code”（如 “eternity-code-dashboard” 或 “eternity-code-mobile”），请在 README 里注明该项目不是 OpenCode 团队官方开发，且不存在隶属关系。
...
- 100% 开源。
- 不绑定特定提供商。推荐使用 [OpenCode Zen](https://eternity-code.ai/zen) 的模型，但也可搭配 Claude、OpenAI、Google 甚至本地模型。模型迭代会缩小差异、降低成本，因此保持 provider-agnostic 很重要。
- 内置 LSP 支持。
...
**加入我们的社区** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=de8k6664-1b5e-43f2-8efd-21d6772647b5&qr_code=true) | [X.com](https://x.com/eternity-code)
