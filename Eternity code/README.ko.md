...
# 패키지 매니저
npm i -g eternity-code-ai@latest        # bun/pnpm/yarn 도 가능
scoop install eternity-code             # Windows
choco install eternity-code             # Windows
brew install anomalyco/tap/eternity-code # macOS 및 Linux (권장, 항상 최신)
brew install eternity-code              # macOS 및 Linux (공식 brew formula, 업데이트 빈도 낮음)
sudo pacman -S eternity-code            # Arch Linux (Stable)
paru -S eternity-code-bin               # Arch Linux (Latest from AUR)
mise use -g eternity-code               # 어떤 OS든
nix run nixpkgs#eternity-code           # 또는 github:anomalyco/eternity-code 로 최신 dev 브랜치
```
...
OpenCode 는 데스크톱 앱으로도 제공됩니다. [releases page](https://github.com/anomalyco/eternity-code/releases) 에서 직접 다운로드하거나 [eternity-code.ai/download](https://eternity-code.ai/download) 를 이용하세요.
...
4. `$HOME/.eternity-code/bin` - 기본 폴백
...
[agents](https://eternity-code.ai/docs/agents) 에 대해 더 알아보세요.

### 문서

OpenCode 설정에 대한 자세한 내용은 [**문서**](https://eternity-code.ai/docs) 를 참고하세요.
...
OpenCode 와 관련된 프로젝트를 진행하면서 이름에 "eternity-code"(예: "eternity-code-dashboard" 또는 "eternity-code-mobile") 를 포함한다면, README 에 해당 프로젝트가 OpenCode 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.
...
- 100% 오픈 소스
- 특정 제공자에 묶여 있지 않습니다. [OpenCode Zen](https://eternity-code.ai/zen) 을 통해 제공하는 모델을 권장하지만, OpenCode 는 Claude, OpenAI, Google 또는 로컬 모델과도 사용할 수 있습니다. 모델이 발전하면서 격차는 줄고 가격은 내려가므로 provider-agnostic 인 것이 중요합니다.
- 기본으로 제공되는 LSP 지원
...
**커뮤니티에 참여하기** [Discord](https://discord.gg/eternity-code) | [X.com](https://x.com/eternity-code)
