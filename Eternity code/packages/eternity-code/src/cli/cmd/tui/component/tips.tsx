...
const TIPS = [
  ...
  "Run {highlight}/share{/highlight} to create a public link to your conversation at eternity-code.ai",
  ...
  "Create {highlight}eternity-code.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/eternity-code/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Eternity Code auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.eternity-code/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.eternity-code/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "Eternity Code auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "Eternity Code uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.eternity-code/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.eternity-code/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Eternity Code from reading sensitive files",
  "Use {highlight}eternity-code run{/highlight} for non-interactive scripting",
  "Use {highlight}eternity-code --continue{/highlight} to resume the last session",
  "Use {highlight}eternity-code run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}eternity-code serve{/highlight} for headless API access to Eternity Code",
  "Use {highlight}eternity-code run --attach{/highlight} to connect to a running server",
  "Run {highlight}eternity-code upgrade{/highlight} to update to the latest version",
  "Run {highlight}eternity-code auth list{/highlight} to see all configured providers",
  "Run {highlight}eternity-code agent create{/highlight} for guided agent creation",
  "Use {highlight}/eternity-code{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}eternity-code github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/eternity-code fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.eternity-code/themes/{/highlight} directory",
  ...
  "Run {highlight}eternity-code debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/eternity-code{/highlight} for containerized use",
  "Use {highlight}/connect{/highlight} with Eternity Code Zen for curated, tested models",
