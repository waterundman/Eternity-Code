...
export const UninstallCommand = {
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall Eternity Code")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g eternity-code.ai",
      pnpm: "pnpm uninstall -g eternity-code.ai",
      bun: "bun remove -g eternity-code.ai",
      yarn: "yarn global remove eternity-code.ai",
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string[]> = {
      ...
      bun: ["bun", "remove", "-g", "eternity-code.ai"],
      yarn: ["yarn", "global", "remove", "eternity-code.ai"],
      brew: ["brew", "uninstall", "eternity-code"],
      choco: ["choco", "uninstall", "eternity-code"],
      scoop: ["scoop", "uninstall", "eternity-code"],
    }

    const cmd = cmds[method]
    if (cmd) {
      spinner.start(`Running ${cmd.join(" ")}...`)
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "eternity-code", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  if (method === "curl" && targets.binary) {
    ...
    if (binDir.includes(".eternity-code")) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using Eternity Code!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# opencode") || content.includes(".eternity-code/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".eternity-code/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".eternity-code/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".eternity-code"))
    ) {
      continue
    }
