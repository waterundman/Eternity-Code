...
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@eternity-code/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run eternity-code with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running eternity-code server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      const headers = (() => {
        ...
        const username = process.env.ETERNITY_CODE_SERVER_USERNAME ?? "eternity-code"
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createOpencodeClient({ baseUrl: "http://eternity-code.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
