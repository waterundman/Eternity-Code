...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run eternity-code",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.code === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for eternity-code session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found eternity-code session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await Process.text(["eternity-code", "import", sessionUrl], {
                  nothrow: true,
                })
        ...
        UI.println()
        UI.println("Starting eternity-code...")
        UI.println()

        const eternityCodeArgs = sessionId ? ["-s", sessionId] : []
        const eternityCodeProcess = Process.spawn(["eternity-code", ...eternityCodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
        })
        const code = await eternityCodeProcess.exited
        if (code !== 0) throw new Error(`eternity-code exited with code ${code}`)
      },
    })
  },
})
