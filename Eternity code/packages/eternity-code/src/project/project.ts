...
import { fn } from "@eternity-code/util/fn"
...
export namespace Project {
  ...
  function readCachedId(dir: string) {
    return Filesystem.readText(path.join(dir, "eternity-code"))
      .then((x) => x.trim())
      .then(ProjectID.make)
      .catch(() => undefined)
  }
      ...
      if (dotgit) {
        ...
        if (!id) {
          ...
          if (id) {
            ...
            await Filesystem.write(path.join(worktree, ".git", "eternity-code"), id).catch(() => undefined)
          }
        }
