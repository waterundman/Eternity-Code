...
import type { AssistantMessage, Event, OpencodeClient, SessionMessageResponse, ToolPart } from "@eternity-code/sdk/v2"
...
export namespace ACP {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `eternity-code auth login` in the terminal",
        name: "Login with eternity-code",
        id: "eternity-code-login",
      }
      ...
      if (params.clientCapabilities?._meta?.["terminal-auth"] === true) {
        authMethod._meta = {
          "terminal-auth": {
            command: "eternity-code",
            args: ["auth", "login"],
            label: "Eternity Code Login",
          },
        }
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // Eternity Code stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string): Promise<{ providerID: ProviderID; modelID: ModelID }> {
    ...
    const eternityCodeProvider = providers.find((p) => p.id === "eternity-code")
    if (eternityCodeProvider) {
      if (eternityCodeProvider.models["big-pickle"]) {
        return { providerID: ProviderID.make("eternity-code"), modelID: ModelID.make("big-pickle") }
      }
      const [best] = Provider.sort(Object.values(eternityCodeProvider.models))
      if (best) {
        return {
