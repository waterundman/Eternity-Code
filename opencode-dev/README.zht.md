...
# 套件管理員
npm i -g eternity-code-ai@latest        # 也可使用 bun/pnpm/yarn
scoop install eternity-code             # Windows
choco install eternity-code             # Windows
brew install anomalyco/tap/eternity-code # macOS 與 Linux（推薦，始終保持最新）
brew install eternity-code              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S eternity-code            # Arch Linux (Stable)
paru -S eternity-code-bin               # Arch Linux (Latest from AUR)
mise use -g eternity-code               # 任何作業系統
nix run nixpkgs#eternity-code           # 或使用 github:anomalyco/eternity-code 以取得最新開發分支
```
...
OpenCode 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://github.com/anomalyco/eternity-code/releases) 或 [eternity-code.ai/download](https://eternity-code.ai/download) 下載。
...
4. `$HOME/.eternity-code/bin` - 預設備用路徑
...
了解更多關於 [Agents](https://eternity-code.ai/docs/agents) 的資訊。

### 線上文件

關於如何設定 OpenCode 的詳細資訊，請參閱我們的 [**官方文件**](https://eternity-code.ai/docs)。
...
如果您正在開發與 OpenCode 相關的專案，並在名稱中使用了 "eternity-code"（例如 "eternity-code-dashboard" 或 "eternity-code-mobile"），請在您的 README 中加入聲明，說明該專案並非由 OpenCode 團隊開發，且與我們沒有任何隸屬關係。
...
- 100% 開源。
- 不綁定特定的服務提供商。雖然我們推薦使用透過 [OpenCode Zen](https://eternity-code.ai/zen) 提供的模型，但 OpenCode 也可搭配 Claude, OpenAI, Google 甚至本地模型使用。隨著模型不斷演進，彼此間的差距會縮小且價格會下降，因此具備「不限廠商 (provider-agnostic)」的特性至關重要。
- 內建 LSP (語言伺服器協定) 支援。
...
**加入我們的社群** [飞书](https://applink.feishu.cn/client/chat/chatter/add_by_link?link_token=de8k6664-1b5e-43f2-8efd-21d6772647b5&qr_code=true) | [X.com](https://x.com/eternity-code)
