    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (servers.length === 0) {
          ...
          prompts.outro("Add servers with: eternity-code mcp add")
          return
        }
    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in eternity-code.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .eternity-code/ subdirectory too)
  const candidates = [path.join(baseDir, "eternity-code.json"), path.join(baseDir, "eternity-code.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".eternity-code", "eternity-code.json"), path.join(baseDir, ".eternity-code", "eternity-code.jsonc"))
  }
  ...
  // Default to eternity-code.json if none exist
  return candidates[0]
}
      ...
      async fn() {
        ...
        if (type === "local") {
          const command = await prompts.text({
            ...
            placeholder: "e.g., eternity-code x @modelcontextprotocol/server-filesystem",
            validate: (x) => (x && x.length > 0 ? undefined : "Required"),
          })
          ...
          const response = await fetch(serverConfig.url, {
            ...
            body: JSON.stringify({
              ...
              params: {
                ...
                capabilities: {},
                clientInfo: { name: "eternity-code-debug", version: Installation.VERSION },
              },
              id: 1,
            }),
          })
