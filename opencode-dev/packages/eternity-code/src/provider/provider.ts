...
export namespace Provider {
  ...
  const CUSTOM_LOADERS: Record<string, CustomLoader> = {
    ...
    async opencode(input) {
      const hasKey = await (async () => {
        ...
        if (config.provider?.["eternity-code"]?.options?.apiKey) return true
        return false
      })()
    ...
    "amazon-bedrock": async () => {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>) {
          ...
          // 1. options.region from eternity-code.json provider config
    ...
    openrouter: async () => {
      return {
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://eternity-code.ai/",
            "X-Title": "eternity-code",
          },
        },
      }
    },
    ...
    cerebras: async () => {
      return {
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "eternity-code",
          },
        },
      }
    },
  ...
  export async function getSmallModel(providerID: ProviderID) {
    ...
    if (provider) {
      ...
      if (providerID.startsWith("eternity-code")) {
        priority = ["gpt-5-nano"]
      }
